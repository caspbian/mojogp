"""SingleOutputGP with fixed grouped observation noise."""

import numpy as np

from mojogp import RBF, SingleOutputGP


def make_data(n_train: int = 5000, n_test: int = 256, seed: int = 21):
    rng = np.random.default_rng(seed)
    X_train = np.linspace(-3.0, 3.0, n_train, dtype=np.float32).reshape(-1, 1)
    train_groups = (X_train[:, 0] > 0.0).astype(np.int32)
    group_noise = np.array([0.01, 0.05], dtype=np.float32)
    train_noise = group_noise[train_groups]
    latent_train = np.sin(2.0 * X_train[:, 0]).astype(np.float32)
    y_train = (latent_train + rng.normal(0.0, np.sqrt(train_noise))).astype(np.float32)

    X_test = np.linspace(-2.8, 2.8, n_test, dtype=np.float32).reshape(-1, 1)
    test_groups = (X_test[:, 0] > 0.0).astype(np.int32)
    latent_test = np.sin(2.0 * X_test[:, 0]).astype(np.float32)
    return X_train, y_train, train_groups, group_noise, X_test, test_groups, latent_test


def run_example(method: str = "matrix_free", n_train: int = 5000, n_test: int = 256):
    X_train, y_train, train_groups, group_noise, X_test, test_groups, latent_test = make_data(
        n_train=n_train,
        n_test=n_test,
    )

    gp = SingleOutputGP(RBF(), verbose=False)
    gp.fit(
        X_train,
        y_train,
        noise_model="grouped",
        noise_group_train=train_groups,
        group_noise=group_noise,
        learn_noise=False,
        method=method,
        max_iterations=20,
        learning_rate=0.03,
        num_probes=4,
        max_cg_iterations=50,
        preconditioner_rank=10,
    )

    latent = gp.predict_latent(X_test, variance_method="mean_only")
    observed = gp.predict_observed(
        X_test,
        noise_group_test=test_groups,
        variance_method="exact",
        max_cg_iterations=80,
        preconditioner_rank=10,
    )
    rmse = float(np.sqrt(np.mean((latent.mean - latent_test) ** 2)))

    print(f"Method: {method}")
    print(f"Latent RMSE: {rmse:.4f}")
    print(f"Group noise: {group_noise.tolist()}")
    print(f"Observed std mean: {float(np.mean(observed.std)):.4f}")
    print(f"Train route metadata: {gp.backend_train_info}")


if __name__ == "__main__":
    run_example("materialized")
    run_example("matrix_free")
